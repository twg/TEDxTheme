...
<input type="hidden" name="_schedule_item_type_nonce" value="<?php echo wp_create_nonce('tedx_schedule_item_type_nonce'); ?>"/>
...
<div class="speaker-list">
  ...
  <select multiple="multiple" name="_schedule_item_speaker_ids[]" id="speaker_ids">
    <?php foreach ($speakers as $key => $speaker): ?>
      ...
      ?>
      <option value="<?php echo $speaker->ID; ?>" <?php if ($selected): ?> selected="selected"<?php endif; ?>><?php echo $speaker->post_title; ?></option>
    <?php endforeach; ?>
  </select>

  <p class="description">Select the speaker(s) of this schedule item</p>
</div>
<input type="hidden" name="_schedule_item_speaker_ids_nonce" value="<?php echo wp_create_nonce('tedx_schedule_item_speaker_ids_nonce'); ?>"/>

<div class="other-input">
  <h4>Event Label</h4>
  <input type="text" id="schedule_item_other_label" name="_schedule_item_other_label" value="<?php echo $schedule_item_other_label ?>">

  <p class="description">The label for this schedule item (eg. Lunch Break)</p>
</div>
<input type="hidden" name="_schedule_item_other_label_nonce" value="<?php echo wp_create_nonce('tedx_schedule_item_other_label_nonce'); ?>"/>
